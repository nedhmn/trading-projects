WITH
            ...
            *,
            ROW_NUMBER() OVER (
                PARTITION BY
                    ...
                    expir_date
                ORDER BY
                    ABS(put_delta - (-0.02)) -- Selecting 2 delta puts
            ) AS rn
        FROM
            dte_7_data
    ),
    selected_pairs AS (
        SELECT
            c.call_id,
            p.put_id,
            tp.put_id AS tail_put_id,
            c.trade_date AS signal_date,
            c.expir_date,
            c.call_delta,
            p.put_delta,
            tp.put_delta AS tail_put_delta,
            c.strike_price AS call_strike,
            p.strike_price AS put_strike,
            tp.strike_price AS tail_put_strike,
            c.stock_price
        FROM
            call_selection c
            ...
            AND c.rn = 1
            AND p.rn = 1
            INNER JOIN tail_put_selection tp ON c.trade_date = tp.trade_date
            AND c.expir_date = tp.expir_date
            AND tp.rn = 1
    ),
    option_prices AS (
        SELECT
            ...
            (tp.put_bid_price + tp.put_ask_price) / 2 AS tail_put_mid_price,
            c.delta AS call_delta,
            (p.delta - 1) AS put_delta,
            (tp.delta - 1) AS tail_put_delta
        FROM
            selected_pairs sp
            ...
            INNER JOIN `nedhmn.option_prices.spy_data` tp ON sp.tail_put_id = tp.put_id
            AND tp.trade_date = c.trade_date
        WHERE
            c.trade_date BETWEEN sp.signal_date AND sp.expir_date
    ),
    prices_with_positions AS (
        SELECT
            *,
            call_mid_price + put_mid_price AS strangle_price,
            tail_put_mid_price AS tail_hedge_price,
    ...
    entry_prices AS (
        SELECT
            ...
            tail_put_mid_price AS tail_put_mid_price_entry,
            call_delta AS call_delta_entry,
            put_delta AS put_delta_entry,
            tail_put_delta AS tail_put_delta_entry,
            strangle_price AS strangle_price_entry,
            tail_hedge_price AS tail_hedge_price_entry
        FROM
    ...
    exit_prices AS (
        SELECT
            ...
            tail_put_mid_price AS tail_put_mid_price_exit,
            strangle_price AS strangle_price_exit,
            tail_hedge_price AS tail_hedge_price_exit
        FROM
            prices_with_positions
        WHERE
            dte = min_dte
    ...
    position_pnl AS (
        SELECT
            ...
            x.tail_put_mid_price_exit,
            x.strangle_price_exit,
            x.tail_hedge_price_exit
        FROM
            entry_prices e
...
SELECT
    ...
    ROUND(tail_put_delta_entry, 5) AS tail_put_delta_entry,
    ROUND(strangle_price_entry, 3) AS strangle_price_entry,
    ROUND(strangle_price_exit, 3) AS strangle_price_exit,
    ROUND(tail_hedge_price_entry, 3) AS tail_put_price_entry,
    ROUND(tail_hedge_price_exit, 3) AS tail_put_price_exit
FROM
    position_pnl
WHERE
